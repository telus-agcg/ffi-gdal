...
RSpec.shared_examples 'a simple curve geometry' do
  ...
  describe '#add_point' do
    context 'normal number values' do
      it 'adds the point to the geometry' do
        expect { subject.add_point(123.456, 6.54) }
    ...
    context 'bad number values' do
      it 'raises' do
        expect { subject.add_point('meow', 'pants') }.to raise_exception TypeError
      end
    end
  end
  ...
  describe '#point' do
    context 'point at index exists' do
      it 'adds the point to the geometry' do
        expect(subject.point(0)).to contain_exactly(an_instance_of(Float), an_instance_of(Float))
      end
    end
  ...
  describe '#set_point' do
    context 'normal number values' do
      it 'adds the point to the geometry' do
        expect { subject.set_point(0, 123.456, 6.54) }
          .to change { subject.point(0) }
          .from([an_instance_of(Float), an_instance_of(Float)])
          .to([123.456, 6.54])
      end
    end
    ...
    context 'bad index value' do
      it 'raises' do
        expect { subject.set_point(-1, 42, 42) }.to raise_exception GDAL::UnsupportedOperation
      end
    end

    context 'non-numbers for coordinate values' do
      it 'raises' do
        expect { subject.set_point(0, 'stuff', 'things') }.to raise_exception TypeError
      end
    end
  end
  ...
  describe '#points' do
    ...
    # is happy there.
    it 'returns a 2-element Array for each point' do
      expect(subject.points.first).to contain_exactly(an_instance_of(Float), an_instance_of(Float))
    end
  end
