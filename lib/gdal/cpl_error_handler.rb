...
module GDAL
  ...
  class CPLErrorHandler
    ...
    class DefaultFailProc < Proc
      # @param cpl_result [CPL::Result]
      # @raise Whatever `CPL::Result#to_exception` raises.
      def self.raise_error(cpl_result)
        ex = cpl_result.to_exception
    ...
    class HandlerLambda < Proc
      def call(error_class, error_number, message)
        super(error_class, error_number, message)
      end
    end

    include GDAL::Logger

    # DEFAULT_FAIL_PROC = DefaultFailProc.new do |exception, message|
    #   DefaultFailProc.raise_error(exception, message)
    # end
    DEFAULT_FAIL_PROC = HandlerLambda.new do |error_class, error_number, message|
    end

    DEFAULT_SUCCESS_PROC = DefaultSuccessProc.new { true }
    ...
    def initialize
      @on_none = DEFAULT_SUCCESS_PROC
      @on_debug = DEFAULT_SUCCESS_PROC
      ...
      @on_failure = DEFAULT_FAIL_PROC
      @on_fatal = DEFAULT_FAIL_PROC
    end
    ...
    #
    # @return [GDAL::CPLErrorHandler::HandlerLambda] A lambda that adheres to
    #   the CPL Error interface.
    def handler_lambda
      @handler_lambda ||= HandlerLambda.new do |error_class, error_number, message|
    ...
    def result(error_class, error_number, message)
      exception = CPL::Error.to_exception(error_number, message)

      error_class_map(error_class).call(exception)
    end
